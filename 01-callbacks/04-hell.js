...
const purchase = (budgetAmount) => {
  const wallet = budget(budgetAmount);
  let amount = 0;

  const buy = (subtotal) => {
    const processWithdraw = (success) => {
    ...
    wallet.withdraw(subtotal, processWithdraw);
  };

  const buyGroup = (group) => {
    groupTotal(group, buy);
  };

  const iteratePurchase = (purchase) => {
    iterateGroups(purchase, buyGroup);
  };

  getPurchase(iteratePurchase);
};
